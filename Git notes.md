...
`git branch <имя ветки>` - создать ветку с последнего коммита
команда `git branch` покажет список существующих веток в репозитории.
`git checkout <имя ветки>` - переключиться на определенную ветку
...
`git rebase main <имя ветки>` - перестраивает коммиты с соседствующей ветки на ветку main, при этом создавая коммиты после всех существующих на ветке main.
```
Пример: ветка main: A -> B -> C -> D
ветка, из которой хотим сделать rebase берет начало из коммита B: E -> F -> G

После команды rebase ветка main будет выглядеть следующим образом:
A -> B -> C -> D --> E -> F -> G

При этом ветка откуда переносим коммиты в main будет удалена.
```
После переноса коммитов из одной ветки в другую слияния как такого не произойдет, все еще можно будет переключиться
между ветками. Хотя и предыдущая ветка удаляется. Для полного слияния необходимо выполнить из ветки main `git merge <имя сливаемой с main ветки>`.

Удалить ненужную ветку.
1. Локальный реп: `git branch -d <имя ветки для удаления>`
2. Удаленный реп: `git push origin -d <имя ветки для удаления>`
